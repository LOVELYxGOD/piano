...
while True:
    ...
    if mode == "play":
        if is_play:  # только если сейчас идёт игра - нет проигрыша или победы
            is_created_notes = created_notes < len(playing_song.notes)
            diff_time = time.time() - timer
            creating_interval = playing_song.interval * playing_song.duration[created_notes]

            if is_created_notes and diff_time >= creating_interval:
